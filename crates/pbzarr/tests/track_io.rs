...
fn two_contig_store(path: &std::path::Path) -> PbzStore {
    ...
    store
        .create_track("depth", g, TrackConfig::new(Dtype::I32).chunk_size(10))
        .unwrap();
    store
}
...
fn write_and_read_region_translates_offset() {
    ...
    // Write [95,115) on chr2 (flat 1095..1115), spanning chunk boundaries at
    // 1100 and 1110.
    let region = store
        .genome_for("depth")
        .unwrap()
        .resolve(&"chr2:95-115".parse().unwrap())
        .unwrap();
    ...
    let chr1 = store
        .genome_for("depth")
        .unwrap()
        .resolve(&"chr1:95-115".parse().unwrap())
        .unwrap();
    let other: ArrayD<i32> = track.read_region(&chr1).unwrap();
    assert_eq!(other.into_raw_vec_and_offset().0, vec![0i32; 20]);
}

#[test]
fn read_and_write_region_past_contig_end_errors() {
    let dir = TempDir::new().unwrap();
    let path = dir.path().join("s.pbz");
    let store = two_contig_store(&path);
    let track = store.track("depth").unwrap();
    ...
    // chr2's seeded data must be untouched.
