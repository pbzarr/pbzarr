...
/// accepted formats.
pub fn parse_region_query(region: &str) -> Result<RegionQuery> {
    region.parse()
}
...
mod tests {
    ...
    #[test]
    fn contig_only() {
        let r = parse_region_query("chr1").unwrap();
        assert_eq!(r.contig, "chr1");
    ...
    #[test]
    fn range() {
        let r = parse_region_query("chr1:1000-2000").unwrap();
        assert_eq!(r.contig, "chr1");
        assert_eq!(r.start, Some(1000));
        assert_eq!(r.end, Some(2000));
    }
    ...
    #[test]
    fn single_position() {
        let r = parse_region_query("chr1:5000").unwrap();
        assert_eq!(r.contig, "chr1");
    ...
    fn comma_separated_numbers() {
        let r = parse_region_query("chr1:1,000,000-2,000,000").unwrap();
        assert_eq!(r.contig, "chr1");
    ...
    fn whitespace_trimming() {
        let r = parse_region_query("  chr1:100-200  ").unwrap();
        assert_eq!(r.contig, "chr1");
    ...
    #[test]
    fn zero_start() {
        let r = parse_region_query("chr1:0-1000").unwrap();
        assert_eq!(r.contig, "chr1");
    ...
    #[test]
    fn err_empty() {
        assert!(parse_region_query("").is_err());
        assert!(parse_region_query("   ").is_err());
    }

    #[test]
    fn err_missing_contig() {
        assert!(parse_region_query(":1000-2000").is_err());
    }

    #[test]
    fn err_invalid_number() {
        assert!(parse_region_query("chr1:abc-2000").is_err());
        assert!(parse_region_query("chr1:1000-xyz").is_err());
        assert!(parse_region_query("chr1:abc").is_err());
    }

    #[test]
    fn err_inverted_range() {
        assert!(parse_region_query("chr1:2000-1000").is_err());
    }

    #[test]
    fn err_equal_start_end() {
        assert!(parse_region_query("chr1:1000-1000").is_err());
    }
    ...
    fn display_round_trip() {
        ...
        for input in cases {
            let r: RegionQuery = input.parse().unwrap();
