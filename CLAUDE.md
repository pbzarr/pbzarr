...
- **Parallel-writer pipeline** in `pbzarr::import::pipeline`. `thread::scope` + a single bounded `crossbeam-channel` of tasks, one task per physical write-unit (a `ChunkTask` is a chunk-aligned `[start, end)` segment of the flat position axis at full column width; when sharded, that unit is the shard, since the shard is the chunk at the `Array` level). Each worker forks readers, then does read+write itself — no writer thread. Shared `Arc<State>` with `AtomicU64` counters and a `Mutex<Option<PbzError>>` for first error.
- **Import tasks partition by physical write-unit, not by contig.** Task boundaries land on the track's on-disk chunk grid, so every task writes one whole chunk (or shard) that no other task touches — zarrs stays on its single-encode path, there is no concurrent read-modify-write hazard, and the writes run fully in parallel (no write lock). A task may straddle contig boundaries; `process_task` fills each overlapping contig's slice of the buffer by name via `Track::write_flat` on the flat axis. `run_pipeline` always steps by `track.chunk_size()` (the physical write-unit); `Config::chunk_size`/`column_chunk_size` are consumed at track creation by `from_d4`/`from_bigwig`, not by the stepping.
- **`run_pipeline<T, R>` takes `&Track`.** No `&mut`. `Track` caches its `values` array handle once (`RwLock<Option<Arc<Array<dyn ReadableWritableListableStorageTraits>>>>>`, one array now that a track is a single flat array rather than one array per contig) so per-chunk reads/writes don't re-open `zarr.json`. `Genome::offsets()` gives the cumulative-offset table `run_pipeline` uses to place each task on the flat axis.
...
- **Within one write-unit, the buffer read-fill is serial.** A task reads its whole region on one thread (outer loop over overlapping contigs, inner loop over columns) before the single whole-unit write. Cross-task parallelism covers this whenever write-units outnumber workers, which holds for the non-sharded default. It only under-uses cores when write-units are few and large (the sharded regime), where a column-parallel fill would help; deferred with the sharding-default decision (see Deferred).
...
- `pbz` CLI binary.
- Multi-column BED into a single wide 2D track (vs the several-scalar-tracks form `from_bed_multi` implements today), and dtype inference / auto-schema from the header.
